...
/** PROTOTYPE chrome — floating switcher for the design variants (?design=A|B|C). */
export const DesignSwitcher = ({
    ...
    const idx = Math.max(0, DESIGN_META.findIndex((m) => m.key === current));
    ...
    useEffect(() => {
        const onKey = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowLeft') {
                e.preventDefault();
                onSelect(DESIGN_META[(idx - 1 + DESIGN_META.length) % DESIGN_META.length].key);
            } else if (e.key === 'ArrowRight') {
                e.preventDefault();
                onSelect(DESIGN_META[(idx + 1) % DESIGN_META.length].key);
            }
        };
    ...
    return (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[12000] flex items-center gap-1 bg-slate-900/95 text-white rounded-full pl-2 pr-1 py-1.5 shadow-2xl border border-slate-700">
            <button
                onClick={() => onSelect(DESIGN_META[(idx - 1 + DESIGN_META.length) % DESIGN_META.length].key)}
            ...
            <span className="px-2 text-xs font-bold whitespace-nowrap">
                {current} — {DESIGN_META[idx].name}
            </span>
            <button
                onClick={() => onSelect(DESIGN_META[(idx + 1) % DESIGN_META.length].key)}
                className="px-2 py-1 rounded-full hover:bg-slate-700 text-lg leading-none"
                title="Next variant"
            >
                ›
